...
// 这个函数使用给定的数组和大小来初始化一个循环缓冲区。
// 参数：
// - CBuf: 一个指向 CircBuf_t 结构体的指针，表示循环缓冲区
// - Buff: 一个指向无符号字符数组的指针，作为循环缓冲区的存储空间
// - Size: 一个无符号整数，表示数组的大小
// 返回值：
// - 循环缓冲区的实际大小，以整数形式返回，或者如果发生错误，返回 -1
#define MAX_POWER_OF_TWO (INT_MAX ^ UINT_MAX)
void CircBuf_Init(CircBuf_t *CBuf, unsigned char *Buff, unsigned int Size)
{
    CBuf->Buffer = Buff;
 ...
 */
void CircBuf_Drop(CircBuf_t *CBuf,  int LenToDrop)
{
